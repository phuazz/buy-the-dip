...
- **2026-07-04 (allocator summary)** — Plain-language companion to the technical record filed: `reviews/2026-07-04_phase1-2b-3b_full-history_summary.docx` (build `scripts/build_phase123_summary.js`). Chart-led, non-technical; answers directly why this project's reconstruction reads lower than the published CrackingMarkets "Buy the dip (weekly)" page — the engine is verified and the mechanism reproduces, and the headline gap is in-sample-versus-out-of-sample honesty (the weekly variant failed its single locked exam), the free-slot participation/fill convention, and the source's non-compounding fixed-capital accounting. Reuses the four filed charts; introduces no new numbers.
