...
- **Phase 0 (trial window)** — smoke test, plumbing, engine validation on ~2 years of Norgate data. Gate: engine mechanics verified; decision on Platinum subscription.
- **Phase 1 (full history)** — replicate the published daily baseline 2000→. Gate: trade count and win rate within tolerance of the anchors (~25,000 / 56.81%).
- **Phase 2 (portfolio/weekly)** — design our own weekly variant: regime filter, weekly dip trigger, volatility ranking (both directions), 10 × 10% slots, 10% stop, profit-target exits, IB-style costs, next-open entry sensitivity.
- **Phase 3 (robustness)** — walk-forward, parameter plateaus, cost sensitivity, regime slices (2008, 2020, 2022); filed review per the studies ledger.
- **Phase 4 (dashboard / multi-strategy)** — template.html + pipeline per `C:\dev\design.md`; publish derived signal JSON to GitHub Pages.
